...
void read_particle_data(Params & parameters, std::vector<Particle> & particles){
    ...
    if (input.is_open()){
        for(int i=0; i<num_particles; ++i){
            ...
            tmp.radius = stod( line.substr(first, second) );
            first = line.find_first_not_of(std::string(" "), second);
            second = line.find(" ", first);
...
__global__ void calc_pos(const Params* params, Particle* particles) { 
    ...
    if(id < params->num_part){
        ...
        if(particles[id].m != -1.){
            ...
            if(x < params->x_min && !params->reflect_x){
                particles[id].x0 = x + (params->x_max - params->x_min);
            }else if(x > params->x_max && !params->reflect_x){
                particles[id].x0 = x - (params->x_max - params->x_min);
            }else{
                particles[id].x0 = x;
            }
            ...
            if(y < params->y_min && !params->reflect_y){
                particles[id].x1 = y + (params->y_max - params->y_min);
            }else if(y > params->y_max && !params->reflect_y){
                particles[id].x1 = y - (params->y_max - params->y_min);
            }else{
                particles[id].x1 = y;
            }
            ...
            if(z < params->z_min && !params->reflect_z){
                particles[id].x2 = z + (params->z_max - params->z_min);
            }else if(z > params->z_max && !params->reflect_z){
                particles[id].x2 = z - (params->z_max - params->z_min);
            }else{
                particles[id].x2 = z;
            }
